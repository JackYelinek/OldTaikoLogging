...
namespace TaikoLoggingTests
{
    ...
    public class TestGoogleSheetInterface
    {
        ...
        public void TestAddRankedMatch()
        {
            ...
            var removedValues = sheet.GetValues("Ranked Logs!A2:S2");
        ...
        [TestMethod]
        public void TestUpdatePS4HighScore()
        {
            // These 2 lines need to be at the start of every test
            sheet.spreadsheetId = spreadsheetId;
            sheet.test = true;

            var Headers = sheet.GetHeaders("Oni");
            var oldValues = sheet.GetValues("Oni!A2:" + sheet.GetColumnName(Headers.Count));
            ...
            var values = sheet.GetValues("Oni!A2:" + sheet.GetColumnName(Headers.Count));
            ...
            List<object> expectedInfo = new List<object>
            {
                ...
                "999",
                "999",
                DateTime.Now.ToString("MM/dd/yyyy"), // This would technically crash if the date changes between the time I update the test and here, but I'd say that doesn't actually matter
            };
            List<string> expectedInfoHeaders = new List<string>
            {
                "Title",
            ...
            {
                if (values[i][1].ToString() == "万戈イム－一ノ十")
                {
                    songIndex = i;
                }
            }
            ...
            for (int i = 0; i < expectedInfoHeaders.Count; i++)
            {
                ...
                for (int j = 0; j < Headers.Count; j++)
                {
                    if (Headers[j] == expectedInfoHeaders[i])
                    {
                        if (string.Compare(values[songIndex][j].ToString(), expectedInfo[i].ToString()) == 0)
                        {
                            results[i] = true;
                            break;
                        }
            ...
            sheet.SendData("Oni!A2:" + sheet.GetColumnName(Headers.Count), oldValues);

            for (int i = 0; i < results.Count; i++)
            {
                Assert.IsTrue(results[i], expectedInfoHeaders[i] + " resulted in " + values[songIndex][Headers.IndexOf(expectedInfoHeaders[i])] + ", should've been " + expectedInfo[i]);
            }
        }
